...
- Each step must map to one of the ten known action types: `home`, `move_arm`, `relative_move`, `gripper`, `pickup`, `dropoff`, `pour`, `push`, `thrust`, `throw`. See [Configuration](configuration.md) for the exact contract and a note on where this repo's older docs disagreed with the actual code.
...
2. **Action whitelist**: only the ten known action types are executable; anything else fails that step and aborts the recipe.
...
| Unknown action type | JSON Parser Node | Action not in the whitelist | Step fails, recipe aborts | Use one of the ten valid actions |
